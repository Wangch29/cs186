...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        return new BPlusTreeIterator(this.root.getLeftmostLeaf(), null);
    }
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        LeafNode ln = this.root.get(key);
        return new BPlusTreeIterator(ln, ln.scanGreaterEqual(key));
    }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> o = this.root.put(key, rid);
        if (o.isEmpty()) {
            return;
        }
        ...
        k.add(o.get().getFirst());
        c.add(this.root.getPage().getPageNum());
        c.add(o.get().getSecond());
        InnerNode newRoot = new InnerNode(this.metadata, this.bufferManager, k, c, this.lockContext);
        updateRoot(newRoot);
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        private Optional<LeafNode> currentNode;
        private Iterator<RecordId> LNIt;

        BPlusTreeIterator(LeafNode currentNode, Iterator<RecordId> it) throws IllegalArgumentException {
            if (currentNode == null) {
                throw new IllegalArgumentException("CurrentNode is null.");
            }
            this.currentNode = Optional.of(currentNode);
            if (it == null) {
                this.LNIt = currentNode.scanAll();
            } else {
                this.LNIt = it;
            }
        }
        ...
        public boolean hasNext() {
            if (this.LNIt.hasNext()) {
                return true;
            }
            while (this.currentNode.flatMap(LeafNode::getRightSibling).isPresent()) {
                this.currentNode = this.currentNode.flatMap(LeafNode::getRightSibling);
                this.LNIt = this.currentNode.get().scanAll();
                if (this.LNIt.hasNext()) {
                    return true;
                }
            }
