...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        minOp = addEligibleSelections(minOp, scanIndex);
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Map.Entry<Set<String>, QueryOperator> entry : prevMap.entrySet()) {
            ...
            QueryOperator prevQO = entry.getValue();
            for (JoinPredicate jp : this.joinPredicates) {
                ...
                if (s.contains(leftTable) && !s.contains(rightTable)) {
                    QueryOperator rightQO = pass1Map.get(Set.of(rightTable));
                    QueryOperator newQO = minCostJoinType(prevQO, rightQO, leftColumn, rightColumn);
                    Set<String> newSet = new HashSet<>(s);
                    newSet.add(rightTable);
                    result.put(newSet, newQO);
                } else if (!s.contains(leftTable) && s.contains(rightTable)) {
                    QueryOperator leftQO = pass1Map.get(Set.of(leftTable));
                    QueryOperator newQO = minCostJoinType(leftQO, prevQO, leftColumn, rightColumn);
                    Set<String> newSet = new HashSet<>(s);
    ...
    public Iterator<Record> execute() {
        ...
        for (String table : this.tableNames) {
            pass1.put(Set.of(table), minCostSingleAccess(table));
        }
        ...
        Map<Set<String>, QueryOperator> passI = pass1;
        while (passI.size() != 1) {
        ...
        this.finalOperator = passI.values().iterator().next();
        this.addGroupBy();
        this.addProject();
        this.addSort();
        this.addLimit();
