...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record record : records) {
            ...
            DataBox columnValue = record.getValue(
                    left ? getLeftColumnIndex() : getRightColumnIndex()
            );
    ...
    public static Pair<List<Record>, List<Record>> getBreakSHJInputs() {
        ...
        for (int i = 0; i < 5 * 4 * 8; ++i) {
            Record kb = createRecord(i);
            leftRecords.add(kb);
        }
        for (int i = 0; i < 8; ++i) {
            Record lj = createRecord(i);
            rightRecords.add(lj);
        }
    ...
    public static Pair<List<Record>, List<Record>> getBreakGHJInputs() {
        ...
        for (int i = 0; i < 6000; ++i) {
            Record kb = createRecord(0);
            leftRecords.add(kb);
        }
        for (int i = 0; i < 6000; ++i) {
            Record lj = createRecord(0);
            rightRecords.add(lj);
        }
